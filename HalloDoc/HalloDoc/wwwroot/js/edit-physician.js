...
const bottom_most_edit = () => {
    const editable = document.getElementsByClassName('bottom-most-editable')
    const disabled = document.getElementsByClassName('remove-disable')
    for (let i = 0; i < disabled.length; ++i) {
        disabled[i].classList.remove("disabled-label")
    }
    for (let i = 0; i < editable.length; ++i) {
        editable[i].disabled = false;
    }
    const bottombtn = document.getElementById("bottom-most-edit")
    const btn = bottombtn.getElementsByClassName('lower-custom-btn')
    for (let i = 0; i < btn.length; ++i) {
        if (btn[i].classList.length == 3) {
            btn[i].classList.remove("hidden");
