...
connection.on("ReceiveMessage", function (reciever, requestId, message, timestamp) {
    const container = document.getElementById(`DPatient-${requestId}-${reciever}`);
...
for (let i = 0; i < chat.length; ++i) {
    chat[i].onclick = function () {
        ...
        document.getElementById("reque-id").value = reqid;
        document.getElementById("aspp-id").value = aspid;
    };
}